...
<body>
  ...
  <div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
      ...
      <div class="container-fluid">
      	<form class="col-12 row mt-5" action="../user/thongke.htm" method="POST">
      		<div class="col-6">
		      <div class="form-group row mt-2">
				  <label for="example-date-input" class=" mx-2 w-25 col-form-label">Từ Ngày</label>
				 
				    <input class="form-control w-50" type="date" value="" id="from" name="from">
				  
				</div>
		
				 <div class="form-group row mt-2">
				  <label for="example-date-input" class=" mx-2 w-25 col-form-label">Đến Ngày</label>
				 
				    <input class="form-control w-50" type="date" value="" id="to" name="to">
				  
				</div>
			</div>
			<div class="form-group row">
			  <button class="btn btn-primary" role="xacnhan"> Xác Nhận </button>
			</div>
     	</form>
     	<div class="h3 font-weight-bolder">Tổng tiền: <span class="text-primary totals">$ </span> </div> 
     	<div class="h4">Tổng đơn:  <span class="text-danger">${list.size() !=0 ?list.size():0 }</span> </div>
  ...
  <script>
	...
	var today = new Date();
	$('#to').val(new Date().toISOString().substr(0, 10));
	$('#from').val(new Date().toISOString().substr(0, 10));
	
	
	var trs = [...$('.table tbody tr')];
	var totals = 0;
	trs.map(tr => {
		totals += parseInt($(tr).children('.total').text());
	} )
	$('.totals').text('$ '+totals);
  </script>
